function status = drawFixWindow(target, errWin, winColor,filledFlag)
...
%     different color.
%   filledFlag determines if filled or open box
%%
...
boxLeft = round(target(1)-errWin);
boxRight = round(target(1)+errWin);
boxTop = round(target(2)-errWin);
boxBottom = round(target(2)+errWin);
...
if filledFlag
    ...
    status = Eyelink('Command', 'draw_box %d %d %d %d %d', ...
        boxLeft, boxTop, boxRight, boxBottom, winColor);
end
