...
% while waitForKeyPress
%     [~, ~, keyCode] = KbCheck;
%     if keyCode(pacmanOpts.KB.space)b
%         waitForKeyPress = false;
%     end
%     WaitSecs(0.001); %so doesn't loop too fast
% end
...
text = strcat(text, 'If you catch one of the prey you gain their points, and move on to the next trial. \n');
...
if pacmanTaskSpecs.gameOpts.timeOutCost == 0
 ...
 text = strcat([text, '\n The bar at the top of the screen indicates how much time you have left. \n']);
% text = strcat([text, '\n Also, make sure to hurry up! The longer you take to capture prey, the less points they are worth!\n']);
text = strcat(text,'\n \n \n');
text = strcat(text,'Press the space bar to continue.');
DrawFormattedText(visEnviro.screen.window, text,'center', 'center',pacmanTaskSpecs.colorOpts.white);
Screen(visEnviro.screen.window,'Flip');
